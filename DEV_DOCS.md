...
`AUTOGENERATED:FUNCTIONS` markers &mdash; change the metadata in `src/interpreter/functionMetadata/` and run
`npm run docs:generate-functions`. CI runs `npm run docs:functions:check` to block a stale table.
