 ...
 * test run by the `test:*` scripts in package.json (each runs `snippets:extract`
 * first), which keeps the docs as the single source of truth and avoids
 * duplicating the snippet code in the repository.
 *
...
function render({ name, lang, code, sourceFile, line }) {
  const banner = [
    ...
    '// run (the `test:*` scripts run `snippets:extract`) and is not committed.',
    '',
