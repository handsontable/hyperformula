...
| Approximate lookup matching across value types    | =XLOOKUP("a", A1:A2, B1:B2, "NF", -1, 1)<br>where A1:A2 = 10, 20 and B1:B2 = "x", "y" | Approximate `MATCH`, `VLOOKUP`, and `HLOOKUP` ignore candidates whose type differs from the lookup value. Approximate `XLOOKUP` follows Microsoft Excel's total ordering across value types in every linear and binary search mode. Returns `"y"` for the example. | Approximate lookups ignore candidates whose type differs from the lookup value. Returns `"NF"` for the example. | Same as HyperFormula. |
