...
| Circular references                                | A1:=A1+1                                                                  | Always returns a `CYCLE` error. (Iterative calculation, which would allow this to converge to a value instead, is not yet part of a released version — see [#1545](https://github.com/handsontable/hyperformula/issues/1545).)                                                                                                                               | Not verified.                                                                                                                  | Iterative calculation is off by default: no cell-level error, the cell silently settles to a computed value (`0` for a simple self-reference) with no way to detect the cycle from the result alone.  |
