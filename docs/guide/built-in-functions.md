...
| SPLIT         | Divides the provided text using space character as a separator and returns the substring at zero-based position specified by the second argument.<br>```SPLIT("Lorem ipsum", 0) -> "Lorem"```<br>```SPLIT("Lorem ipsum", 1) -> "ipsum"``` | SPLIT(Text; Index)                                 |
