...
method. The cell content is not calculated, the method returns only
the type, so, for example, you can check if there is a formula inside
cells. The list of possible cell types: `'FORMULA'`, `'VALUE'`,
`'MATRIX'`, `'EMPTY`.

## Getting cell value type

You can also use the `getCellValueType` the method which returns
the calculated value type, so cell's value for the formula:
`'=SUM(1,2,3)'` will be 'NUMBER'. The list of possible cell value
