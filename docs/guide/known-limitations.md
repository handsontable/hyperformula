...
* INDEX function always returns a single cell. It does not support returning whole rows/columns of the source range. The two first parameters of the INDEX function are required. Only the `Column` parameter can be omitted (it defaults to '1' in such a case).
