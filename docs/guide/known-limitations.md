...
* The six reference-checking branches in the formula parser that build a `#REF!`/`#NAME?` error for a malformed reference (for example a range that spans more than one sheet) report no `originFunction` — the identity is left unattributed rather than mislabeled as a function that only read the value.
