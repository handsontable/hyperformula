...
* Errors that arise inside an array result — a broadcast that has no value for a position, or a per-element type coercion that fails — report no address, and the function whose call the array is passed to may be reported as their `originFunction`. For example, `=INDEX(A1:A2+B1:B3, 3, 1)` reports `INDEX` for an `#N/A` that the `+` operator produced. Errors produced outside an array carry the identity of whatever produced them: a function or operator that rejected a value, `reference` for a reference that cannot be resolved, `removed reference` for one destroyed by removing rows or columns, `parser`, `user input`, or `literal`. `#SPILL!` and `#CYCLE!` report the affected cell with no `originFunction`, because no function produced them.
