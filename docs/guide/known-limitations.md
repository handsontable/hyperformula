...
| Applying a range to a function taking a single value | ABS(A1:B3)                                                                | `WrongType` error.                                                                                                                                                                                             | If there is one cell from the range in the same row/column as the formula, returns its value.<br><br>Otherwise, throws an error. | Fills a number of cells, with the formula in the top-left corner.   |
| Applying an operator to a range                      | A1:A3% A1:A3 + 10                                                         | `ScalarExpected` error.                                                                                                                                                                                        | Same as above.                                                                                                                   | Same as above.                                                      |
