...
differences.forEach((e: [number, number, string, string, string]) => {
  ...
  console.log(`In cell ${sheetCellAddressToString(sheetCellAddress(0, currentColumnIdx, currentRowIdx))} expected '${expectedValue}' but got '${actualValue}'. Original raw cell content: ${formulaString}`)
})
