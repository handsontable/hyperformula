...
export function runBasicBenchmark(): BenchmarkResult[] {
  ...
  batch(result,
    () => benchmark('Sheet A', sheetA, expectedValuesA(sheetA), {numberOfRuns: 64}),
    () => benchmark('Sheet B', sheetB, expectedValuesB(sheetB), {numberOfRuns: 64}),
    () => benchmark('Sheet T', sheetT, expectedValuesT(sheetT), {numberOfRuns: 64}),
    () => benchmark('Column ranges', infiniteRanges, [{
      address: 'AX50',
      value: 1.04519967355127e+63
    }], {expectedTime: 1000, numberOfRuns: 64})
  )
