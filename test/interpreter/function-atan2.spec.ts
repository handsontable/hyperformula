...
describe('Function ATAN2', () => {
  it('happy path', async() => {
const engine = await HyperFormula.buildFromArray([['=ATAN2(1,2)']], { smartRounding : false})

    expect(engine.getCellValue(adr('A1'))).toBeCloseTo(1.107148718, 6)
  })

  it('validates error', async() => {
const engine = await HyperFormula.buildFromArray([['=ATAN2(0,0)']], { smartRounding : false})

    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.DIV_BY_ZERO))
  })

  it('when value not numeric', async() => {
const engine = await HyperFormula.buildFromArray([['=ATAN2(1,"foo")']])

    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.VALUE, ErrorMessage.NumberCoercion))
  })

  it('wrong number of arguments', async() => {
const engine = await HyperFormula.buildFromArray([['=ATAN2()', '=ATAN2(1)']])
