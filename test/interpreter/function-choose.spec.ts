...
const engine = await HyperFormula.buildFromArray([
  ...
  it('Should work with more arguments', async() => {
const engine = await HyperFormula.buildFromArray([
...
const engine = await HyperFormula.buildFromArray([['=CHOOSE(1,B1,3)', '1%' ]])
...
const engine = await HyperFormula.buildFromArray([['=IFERROR(NA(), B1)', '1%' ]])
  ...
  it('Should fail when wrong first argument', async() => {
const engine = await HyperFormula.buildFromArray([
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('Should fail with error in first argument', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=CHOOSE(1/0,3,4,5)']
    ])
    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.DIV_BY_ZERO))
  })
  it('Should not fail with error in other arguments', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=CHOOSE(4,1/0,3,4,5)']
    ])
    expect(engine.getCellValue(adr('A1'))).toEqual(5)
  })
  it('Should pass errors as normal values', async() => {
const engine = await HyperFormula.buildFromArray([
  ...
  it('Should fail with range', async() => {
const engine = await HyperFormula.buildFromArray([
