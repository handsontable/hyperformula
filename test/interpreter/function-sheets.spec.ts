...
describe('Function SHEETS', () => {
  it('should return number of sheets if no argument provided', async() => {
const engine = await HyperFormula.buildFromSheets({
  ...
  it('should return 1 for a valid reference', async() => {
const engine = await HyperFormula.buildFromArray([['=SHEETS(B1)', '=SHEETS(A1:A2)', '=SHEETS(A:B)', '=SHEETS(1:2)']])
  ...
  it('should return VALUE for non-reference parameter', async() => {
const engine = await HyperFormula.buildFromArray([['=SHEETS(1)', '=SHEETS("foo")', '=SHEETS(TRUE())']])
...
const engine = await HyperFormula.buildFromArray([['=SHEETS(1/0)']])

    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.DIV_BY_ZERO))
  })

  it('should work for itself', async() => {
const engine = await HyperFormula.buildFromArray([['=SHEETS(A1)']])
