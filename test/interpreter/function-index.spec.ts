...
const engine = await HyperFormula.buildFromArray([
  ...
  it('requires 2nd and 3rd arguments to be integers', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=INDEX(B1:B1, "foo", 1)'],
      ['=INDEX(B1:B1, 1, "bar")'],
    ])
  ...
  it('requires 2nd argument to be in bounds of range', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=INDEX(B1:D3, -1, 1)'],
      ['=INDEX(B1:D3, 4, 1)'],
      ['=INDEX(42, -1, 1)'],
  ...
  it('requires 2nd and 3rd arguments to be in bounds of range', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=INDEX(B1:D3, 1, -1)'],
      ['=INDEX(B1:D3, 1, 4)'],
      ['=INDEX(42, 1, -1)'],
  ...
  it('works for range and nonzero arguments', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=INDEX(B1:C2, 1, 1)', '1', '2'],
      ['=INDEX(B1:C2, 1, 2)', '3', '4'],
      ['=INDEX(B1:C2, 2, 1)'],
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('should return VALUE error when one of the cooridnate is 0 or null', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=INDEX(B1:D5, 0, 2)'],
      ['=INDEX(B1:D5, 2, 0)'],
      ['=INDEX(B1:D5,,)'],
  ...
  it('should work for scalars too', async() => {
const engine = await HyperFormula.buildFromArray([
      ['foo'],
      ['=INDEX(A1, 1, 1)'],
      ['=INDEX(42, 1, 1)'],
  ...
  it('should assume first column if no last argument', async() => {
const engine = await HyperFormula.buildFromArray([
      [1, 2],
      [3, 4],
      ['=INDEX(A1:B2, 2)'],
