...
const engine = await HyperFormula.buildFromArray([
  ...
  it('coerces only explicit arguments, ignores provided via reference', async() => {
const engine = await HyperFormula.buildFromArray([
  ...
  it('ignores nonnumeric values as long as theres at least one numeric value', async() => {
const engine = await HyperFormula.buildFromArray([
  ...
  it('coerces given string arguments', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=MEDIAN("12", "11", "13")'],
    ])
