...
describe('Function SINH', () => {
  it('happy path', async() => {
const engine = await HyperFormula.buildFromArray([['=SINH(0)', '=SINH(0.5)']])
...
const engine = await HyperFormula.buildFromArray([['=SINH("foo")']])

    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.VALUE, ErrorMessage.NumberCoercion))
  })

  it('wrong number of arguments', async() => {
const engine = await HyperFormula.buildFromArray([['=SINH()', '=SINH(1,-1)']])
