...
const engine = await HyperFormula.buildFromArray([['=CONCATENATE()']])

    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.NA, ErrorMessage.WrongArgNumber))
  })

  it('works', async() => {
const engine = await HyperFormula.buildFromArray([['John', 'Smith', '=CONCATENATE(A1, B1)']])
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('empty value is empty string', async() => {
const engine = await HyperFormula.buildFromArray([
  ...
  it('supports range values', async() => {
const engine = await HyperFormula.buildFromArray([
      ['Topleft', 'Topright'],
