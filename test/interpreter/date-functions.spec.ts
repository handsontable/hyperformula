...
describe('Interpreter', () => {
  it('function DATE with 3 numerical arguments', async () => {
    const engine = await HandsOnEngine.buildFromArray([['=DATE(1900, 1, 1)', '=DATE(1900, 1, 2)', '=DATE(1915, 10, 24)']])

    expect(engine.getCellValue('A1')).toEqual(2)
    expect(dateNumberToString(engine.getCellValue('A1') as number, Config.defaultConfig.dateFormat)).toEqual('01/01/1900')
    expect(engine.getCellValue('B1')).toEqual(3)
    expect(dateNumberToString(engine.getCellValue('B1') as number, Config.defaultConfig.dateFormat)).toEqual('01/02/1900')
    expect(dateNumberToString(engine.getCellValue('C1') as number, Config.defaultConfig.dateFormat)).toEqual('10/24/1915')
  })
  ...
  it('function MONTH with numerical arguments', async () => {
    const engine = await HandsOnEngine.buildFromArray([['=MONTH(0)', '=MONTH(2)', '=MONTH(43465)']])
  ...
  it('function MONTH with string arguments', async () => {
    const engine = await HandsOnEngine.buildFromArray([['=MONTH("12/31/1899")', '=MONTH("01/01/1900")', '=MONTH("12/31/2018")']])
  ...
  it('function YEAR with numerical arguments', async () => {
    const engine = await HandsOnEngine.buildFromArray([['=YEAR(0)', '=YEAR(2)', '=YEAR(43465)']])
  ...
  it('function YEAR with string arguments', async () => {
    const engine = await HandsOnEngine.buildFromArray([['=YEAR("12/31/1899")', '=YEAR("01/01/1900")', '=YEAR("12/31/2018")']])
