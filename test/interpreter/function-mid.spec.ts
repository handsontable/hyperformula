...
describe('Function MID', () => {
  it('should take three arguments', async() => {
const engine = await HyperFormula.buildFromArray([
  ...
  it('should return substring', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=MID("foo", 1, 2)'],
      ['=MID("bar", 2, 2)'],
    ])
  ...
  it('should return empty string if start is greater than text length', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=MID("", 2, 1)'],
      ['=MID("foo", 5, 42)'],
    ])
  ...
  it('should return chars up to string length if end start + number of chars exceeds string length', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=MID("foo", 2, 5)'],
    ])
  ...
  it('should return #VALUE! if start num is less than 1', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=MID("foo", 0, 1)'],
      ['=MID("foo", -1, 1)'],
    ])
  ...
  it('should return #VALUE! if number of chars is negative', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=MID("foo", 1, -1)'],
    ])
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('should return error for range', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=MID(B2:B3, 1, 2)'],
    ])
