...
const engine = await HyperFormula.buildFromArray([
   ...
   */
  it('should accept Rate values that are greater than -1.', async() => {
const engine = await HyperFormula.buildFromArray([
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('only first date needs to be earliest', async() => {
const engine = await HyperFormula.buildFromArray([
  ...
  it('should evaluate to #NUM! if values in range are not numbers', async() => {
const engine = await HyperFormula.buildFromArray([
   ...
   */
  it('should evaluate to #NUM! if ranges are of different length', async() => {
const engine = await HyperFormula.buildFromArray([
  ...
  it('should evaluate to #NUM! if dates are in wrong order', async() => {
const engine = await HyperFormula.buildFromArray([
  ...
  it('should evaluate to #NUM! if dates are too small', async() => {
const engine = await HyperFormula.buildFromArray([
