...
describe('SUMSQ', () => {
  it('SUMSQ without args',  async() => {
const engine = await HyperFormula.buildFromArray([['=SUMSQ()']])

    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.NA, ErrorMessage.WrongArgNumber))
  })

  it('SUMSQ with args', async() => {
const engine = await HyperFormula.buildFromArray([
  ...
  it('SUMSQ with range args',  async() => {
const engine = await HyperFormula.buildFromArray([
      ['1', '2', '5'],
      ['3', '4', '=SUMSQ(A1:B2)'],
    ])
  ...
  it('SUMSQ with using previously cached value',  async() => {
const engine = await HyperFormula.buildFromArray([
      ['3', '=SUMSQ(A1:A1)'],
      ['4', '=SUMSQ(A1:A2)'],
    ])
