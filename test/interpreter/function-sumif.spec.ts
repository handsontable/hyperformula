...
describe('Function SUMIF - argument validations and combinations', () => {
  it('requires 2 or 3 arguments', async() => {
const engine = await HyperFormula.buildFromArray([
  ...
  it('works when 2nd arg is an integer',  async() => {
const engine = await HyperFormula.buildFromArray([
  ...
  it('works when 2nd arg is a boolean',  async() => {
const engine = await HyperFormula.buildFromArray([
  ...
  it('works when 2nd arg is a string "true"',  async() => {
const engine = await HyperFormula.buildFromArray([
      ['=SUMIF(C1:C2, "=TRUE", B1:B2)', 2, 1],
      [null, 3, true],
    ])
  ...
  it('works when 2nd arg is a string "true" in different language', async() => {
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('error when number of elements match but dimensions doesnt',  async() => {
const engine = await HyperFormula.buildFromArray([
      ['=SUMIF(B1:B2, ">0", B1:C1)'],
    ])
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('works for mixed reference/range arguments', async() => {
const engine = await HyperFormula.buildFromArray([
      ['2', '3'],
      ['=SUMIF(A1:A1, ">1", B1)'],
      ['=SUMIF(A1, ">1", B1:B1)'],
  ...
  it('works for 2 arguments', async() => {
const engine = await HyperFormula.buildFromArray([
      ['10', '20', '30'],
      ['=SUMIF(A1:C1, ">15")'],
    ])
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('works with different sheets', async() => {
  ...
  it('works when precision sensitive (default setting)', async() => {
const engine = await HyperFormula.buildFromArray([
        ['1.0000000001', '1'],
        ['1.00000000000005', '2'],
        ['1.00000000000005', '4'],
  ...
  it('criterions are not accent sensitive', async() => {
const engine = await HyperFormula.buildFromArray( [
      ['abcd', '1'],
      ['ABCD', '2'],
      ['abc', '4'],
  ...
  it('criterions are accent sensitive if specified', async() => {
const engine = await HyperFormula.buildFromArray( [
      ['abcd', '1'],
      ['ABCD', '2'],
      ['abc', '4'],
  ...
  it('criterions are not case sensitive', async() => {
const engine = await HyperFormula.buildFromArray( [
      ['abcd', '1'],
      ['ABCD', '2'],
      ['abc', '4'],
  ...
  it('criterions are not case sensitive 2', async() => {
const engine = await HyperFormula.buildFromArray( [
      ['abcd', '1'],
      ['ABCD', '2'],
      ['abc', '4'],
  ...
  it('criterions are case sensitive if specified', async() => {
const engine = await HyperFormula.buildFromArray( [
      ['abcd', '1'],
      ['ABCD', '2'],
      ['abc', '4'],
...
const engine = await HyperFormula.buildFromArray( [
  ...
  it('wildcards instead of regexps', async() => {
const engine = await HyperFormula.buildFromArray( [
      ['a+?*', '1'],
      ['a?*', '2'],
      ['aa~?~*', '4'],
...
describe('Function SUMIFS - argument validations and combinations', () => {
  it('requires odd number of arguments, but at least 3', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=SUMIFS(C1, ">0")'],
      ['=SUMIFS(C1, ">0", B1, B1)'],
    ])
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('coerces dates as numbers', async() => {
const engine = await HyperFormula.buildFromArray([
...
describe('Function SUMIFS - calcultions on more than one criteria', () => {
  it('works for more than one criterion/range pair', async() => {
const engine = await HyperFormula.buildFromArray([
      ['0', '100', '3'],
      ['1', '101', '5'],
      ['2', '102', '7'],
...
describe('Function SUMIF - cache recalculation after cruds', () => {
  it('recalculates SUMIF if changes in summed range', async() => {
    const sheet = [
      ['10', '10'],
      ['5', '6'],
    ...
    const changes = await engine.setCellContents(adr('A1'), [['1', '3']])
  ...
  it('recalculates SUMIF if changes in tested range', async() => {
    const sheet = [
      ['10', '10'],
      ['5', '6'],
  ...
  it('recalculates SUMIF if summed range same as tested range', async() => {
    const sheet = [
      ['10', '10'],
...
describe('Function SUMIFS - cache recalculation after cruds', () => {
  it('recalculates SUMIFS if changes in summed range', async() => {
    const sheet = [
      ['10', '10'],
      ['5', '6'],
      ['7', '8'],
  ...
  it('recalculates SUMIFS if changes in one of the tested range', async() => {
    const sheet = [
      ['10', '10'],
      ['5', '6'],
      ['7', '8'],
    ...
    const changes = await engine.setCellContents(adr('A3'), [['1', '7']])
