...
const engine = await HyperFormula.buildFromArray([
  ...
  it('should return error when value does not correspond to base', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=DECIMAL(12, 2)'],
      ['=DECIMAL("123XYZ", 33)'],
    ])
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('should work for of max length 255', async() => {
  ...
  it('should work only for bases from 2 to 36', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=DECIMAL("0", 0)'],
      ['=DECIMAL("10", 2)'],
      ['=DECIMAL("XYZ", 36)'],
