...
describe('Function CHAR', () => {
  ...
  it('should work only for numbers from 1 to 255', () => {
    ...
    expect(engine.getCellValue(adr('A1'))).toEqual(detailedError(ErrorType.VALUE, ErrorMessages.CharacterCode))
    expect(engine.getCellValue(adr('A2'))).toEqual('')
    expect(engine.getCellValue(adr('A3'))).toEqual('ÿ')
    expect(engine.getCellValue(adr('A4'))).toEqual(detailedError(ErrorType.VALUE, ErrorMessages.CharacterCode))
  })
})
