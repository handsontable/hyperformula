...
const engine = await HyperFormula.buildFromArray([['=MAX()']])
    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.NA, ErrorMessage.WrongArgNumber))
  })

  it('MAX with args',  async() => {
const engine = await HyperFormula.buildFromArray([['=MAX(1, B1)', '3.14']])
    expect(engine.getCellValue(adr('A1'))).toBeCloseTo(3.14)
  })

  it('MAX with range',  async() => {
const engine = await HyperFormula.buildFromArray([['1'], ['3'], ['2'], ['=MAX(A1:A3)']])
    expect(engine.getCellValue(adr('A4'))).toEqual(3)
  })

  it('MAX with mixed arguments',  async() => {
const engine = await HyperFormula.buildFromArray([['1'], ['3'], ['2'], ['=MAX(4,A1:A3)']])
...
const engine = await HyperFormula.buildFromArray([['foo'], ['bar'], ['-1'], ['=MAX(A1:A3)']])
    expect(engine.getCellValue(adr('A4'))).toEqual(-1)
  })

  it('MAX of empty value',  async() => {
const engine = await HyperFormula.buildFromArray([['', '=MAX(A1)']])
    expect(engine.getCellValue(adr('B1'))).toEqual(0)
  })

  it('MAX of empty value and some negative number',  async() => {
const engine = await HyperFormula.buildFromArray([['', '-1', '=MAX(A1,B1)']])
