...
describe('Function LOG', () => {
  it('happy path', async() => {
const engine = await HyperFormula.buildFromArray([['=LOG(4, 2)']])

    expect(engine.getCellValue(adr('A1'))).toBe(2)
  })

  it('logarithmic base has default', async() => {
const engine = await HyperFormula.buildFromArray([['=LOG(10)']])
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('for zero', async() => {
const engine = await HyperFormula.buildFromArray([['=LOG(0, 42)']])

    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.NUM, ErrorMessage.ValueSmall))
  })

  it('for negative value', async() => {
const engine = await HyperFormula.buildFromArray([['=LOG(-42, 42)']])

    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.NUM, ErrorMessage.ValueSmall))
  })

  it('for zero base', async() => {
const engine = await HyperFormula.buildFromArray([['=LOG(42, 0)']])

    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.NUM, ErrorMessage.ValueSmall))
  })

  it('for 1 base', async() => {
const engine = await HyperFormula.buildFromArray([['=LOG(42, 1)']])

    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.NUM, ErrorMessage.NaN))
  })

  it('for negative base', async() => {
const engine = await HyperFormula.buildFromArray([['=LOG(42, -42)']])
...
const engine = await HyperFormula.buildFromArray([['=LOG()', '=LOG(42, 42, 42)']])
