...
describe('computeHashFromTokens', () => {
  ...
  it('simple case', () => {
    ...
    expect(computeFunc(code, adr('B2'))).toBe('=42')
  })

  it('cell relative reference', () => {
    const code = '=A5'

    expect(computeFunc(code, adr('B2'))).toBe('=#3R-1')
  })

  it('cell absolute reference', () => {
    const code = '=$A$5'

    expect(computeFunc(code, adr('B2'))).toBe('=#4A0')
  })

  it('cell absolute col reference', () => {
    const code = '=$A5'

    expect(computeFunc(code, adr('B2'))).toBe('=#3AC0')
  })

  it('cell absolute row reference', () => {
    const code = '=A$5'

    expect(computeFunc(code, adr('B2'))).toBe('=#4AR-1')
  })

  it('more addresses', () => {
    const code = '=A5+A7'

    expect(computeFunc(code, adr('B2'))).toBe('=#3R-1+#5R-1')
  })

  it('cell ref in string', () => {
    const code = '="A5"'

    expect(computeFunc(code, adr('B2'))).toBe('="A5"')
  })

  it('cell ref between strings', () => {
    const code = '="A5"+A4+"A6"'

    expect(computeFunc(code, adr('B2'))).toBe('="A5"+#2R-1+"A6"')
  })

  it('cell ref in string with escape', () => {
    const code = '="fdsaf\\"A5"'

    expect(computeFunc(code, adr('B2'))).toBe('="fdsaf\\"A5"')
  })
  ...
  it('cell range', () => {
    ...
    expect(computeFunc(code, adr('B2'))).toBe('=#3R-1:#14R0')
  })

  it('cell range with sheet on the left', () => {
    const code = '=Sheet1!A5:B16'

    expect(computeFunc(code, adr('B2'))).toBe('=#0#3R-1:#14R0')
  })

  it('cell range with sheet on both sides', () => {
    const code = '=Sheet1!A5:Sheet2!B16'

    expect(computeFunc(code, adr('B2'))).toBe('=#0#3R-1:#1#14R0')
  })

  it('column range', () => {
    const code = '=A:$B'

    expect(computeFunc(code, adr('B2'))).toBe('=#COLR-1:#COLA1')
  })

  it('column range with sheet on the left', () => {
    const code = '=Sheet1!A:B'

    expect(computeFunc(code, adr('B2'))).toBe('=#0#COLR-1:#COLR0')
  })

  it('column range with sheet on both sides', () => {
    const code = '=Sheet1!A:Sheet2!B'

    expect(computeFunc(code, adr('B2'))).toBe('=#0#COLR-1:#1#COLR0')
  })

  it('row range', () => {
    const code = '=1:$2'

    expect(computeFunc(code, adr('B2'))).toBe('=#ROWR-1:#ROWA1')
  })

  it('row range with sheet on the left', () => {
    const code = '=Sheet1!1:2'

    expect(computeFunc(code, adr('B2'))).toBe('=#0#ROWR-1:#ROWR0')
  })

  it('row range with sheet on both sides', () => {
    const code = '=Sheet1!1:Sheet2!2'

    expect(computeFunc(code, adr('B2'))).toBe('=#0#ROWR-1:#1#ROWR0')
  })
  ...
  it('different hash for formulas with different namespace', () => {
    ...
    const result1 = computeFunc(code1, adr('B2'))
    const result2 = computeFunc(code2, adr('B2'))
  ...
  it('support sheets', () => {
    ...
    expect(computeFunc(code, adr('B2'))).toBe('=#1#3R-1')
  })

  it('function call names are normalized', () => {
    const code = '=rAnd()'

    expect(computeFunc(code, adr('B2'))).toBe('=RAND()')
  })

  it('function call in canonical form', () => {
    const code = '=SUMA()'

    expect(computeFunc(code, adr('B2'), 'plPL')).toBe('=SUM()')
  })

  it('function call when missing translation', () => {
    const code = '=fooBAR()'

    expect(computeFunc(code, adr('B2'), 'plPL')).toBe('=FOOBAR()')
  })
  ...
  it('should work with whitespaces', () => {
    ...
    expect(hash).toBe('= - 1 + 2 / 3 - 4 % * (1 + 2 ) + SUM( #0#0R0, #0R0:#1R0 )')
  })
  ...
  it('should not skip whitespaces when there is empty arg', () => {
    ...
    expect(hash).toBe('=PV(#0R0,2,3,   ,#1R0)')
  })
})
