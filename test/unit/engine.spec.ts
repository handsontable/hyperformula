...
describe('#buildFromArray', () => {
  ...
  it('evaluate empty vertex', () => {
    ...
    expect(engine.getCellValue(adr('A5'))).toBeNull()
  ...
  it('should be possible to build graph with reference to not existing sheet', () => {
    ...
    expect(engine.getCellFormula(adr('A1'))).toBe('=Sheet2!A2')
  ...
  it('should propagate parsing errors', () => {
    ...
    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.ERROR, ErrorMessage.ParseError))
    expect(engine.getCellFormula(adr('A1'))).toBe('=SUM(')
...
describe('#getCellHyperlink', () => {
  ...
  it('returns undefined when url not present', () => {
    ...
    expect(engine.getCellHyperlink(adr('A1'))).toBeUndefined()
    expect(engine.getCellHyperlink(adr('B1'))).toBeUndefined()
    expect(engine.getCellHyperlink(adr('C1'))).toBeUndefined()
    expect(engine.getCellHyperlink(adr('D1'))).toBeUndefined()
  })
...
describe('#getCellFormula', () => {
  it('returns formula when present', () => {
    ...
    expect(engine.getCellFormula(adr('A1'))).toBe('=SUM(1,2,3,C3)')
  })
  ...
  it('works with -0', () => {
    ...
    expect(engine.getCellFormula(adr('A1'))).toBe('=-0')
  })
  ...
  it('returns undefined for simple values', () => {
    ...
    expect(engine.getCellFormula(adr('A1'))).toBeUndefined()
    expect(engine.getCellFormula(adr('A2'))).toBeUndefined()
    expect(engine.getCellFormula(adr('A3'))).toBeUndefined()
    expect(engine.getCellFormula(adr('A4'))).toBeUndefined()
  })
  ...
  it('returns matrix formula for matrix vertices', () => {
    ...
    expect(engine.getCellFormula(adr('A3'))).toBe('=MMULT(A1:B2,A1:B2)')
    expect(engine.getCellFormula(adr('A4'))).toBeUndefined()
    expect(engine.getCellFormula(adr('B3'))).toBeUndefined()
    expect(engine.getCellFormula(adr('B4'))).toBeUndefined()
  })
  ...
  it('returns invalid matrix formula literal', () => {
    ...
    expect(engine.getCellValue(adr('A1'))).toEqualError(detailedError(ErrorType.ERROR, ErrorMessage.ParseError))
    expect(engine.getCellFormula(adr('A1'))).toBe('=TRANSPOSE(')
  })
...
describe('#getCellValue', () => {
  it('should return simple value', () => {
    ...
    expect(engine.getCellValue(adr('D1'))).toBe('foo')
    expect(engine.getCellValue(adr('E1'))).toBe(true)
    expect(engine.getCellValue(adr('F1'))).toBe(-1)
  })
...
describe('#getCellSerialized', () => {
  it('should return formula for formula vertex', () => {
    ...
    expect(engine.getCellSerialized(adr('A1'))).toBe('=SUM(1, A2)')
  })
  ...
  it('should return formula for parsing error', () => {
    ...
    expect(engine.getCellSerialized(adr('A1'))).toBe('=SUM(')
  })
  ...
  it('should return simple value', () => {
    ...
    expect(engine.getCellSerialized(adr('A1'))).toBe(1)
    expect(engine.getCellSerialized(adr('B1'))).toBe('2')
    expect(engine.getCellSerialized(adr('C1'))).toBe('foo')
    expect(engine.getCellSerialized(adr('D1'))).toBe(true)
  })
  ...
  it('should return empty value', () => {
    ...
    expect(engine.getCellSerialized(adr('A1'))).toBeNull()
    expect(engine.getCellSerialized(adr('B1'))).toBeNull()
    expect(engine.getCellSerialized(adr('C1'))).toBeNull()
  })
  ...
  it('should return formula of a formula matrix', () => {
    ...
    expect(engine.getCellSerialized(adr('A2'))).toBe('{=TRANSPOSE(A1:B1)}')
    expect(engine.getCellSerialized(adr('A3'))).toBe('{=TRANSPOSE(A1:B1)}')
  })
  ...
  it('should return translated error', () => {
    ...
    expect(engine.getCellSerialized(adr('A1'))).toBe('=#ARG!')
  })
...
describe('#sheetId', () => {
  it('returns id if sheet exists', () => {
    ...
    expect(engine.getSheetId('foobar')).toBe(0)
  })
...
describe('#getCellValueFormat', () => {
  it('non-currency', () => {
    ...
    expect(engine.getCellValueFormat(adr('A1'))).toBeUndefined()
  })

  it('currency', () => {
    const engine = HyperFormula.buildFromArray([['1PLN']], {currencySymbol: ['PLN', '$']})

    expect(engine.getCellValueFormat(adr('A1'))).toBe('PLN')
    expect(engine.getCellValue(adr('A1'))).toBe(1)
  ...
  it('unicode currency', () => {
    ...
    expect(engine.getCellValueFormat(adr('A1'))).toBe('₪')
    expect(engine.getCellValue(adr('A1'))).toBe(1)
    expect(engine.getCellValueDetailedType(adr('A1'))).toEqual(CellValueDetailedType.NUMBER_CURRENCY)
  })
...
describe('#doesCellHaveSimpleValue', () => {
  it('true', () => {
    ...
    expect(engine.doesCellHaveSimpleValue(adr('A1'))).toBe(true)
    expect(engine.doesCellHaveSimpleValue(adr('B1'))).toBe(true)
  })

  it('false', () => {
    const engine = HyperFormula.buildFromArray([['=SUM(1, 2)', null, '=TRANSPOSE(A1:A1)']])

    expect(engine.doesCellHaveSimpleValue(adr('A1'))).toBe(false)
    expect(engine.doesCellHaveSimpleValue(adr('B1'))).toBe(false)
...
describe('#doesCellHaveFormula', () => {
  it('true', () => {
    ...
    expect(engine.doesCellHaveFormula(adr('A1'))).toBe(true)
  })

  it('false', () => {
    const engine = HyperFormula.buildFromArray([['1', '', '{=TRANSPOSE(A1:A1)}', 'foo', null]])

    expect(engine.doesCellHaveFormula(adr('A1'))).toBe(false)
    expect(engine.doesCellHaveFormula(adr('B1'))).toBe(false)
    expect(engine.doesCellHaveFormula(adr('C1'))).toBe(false)
    expect(engine.doesCellHaveFormula(adr('D1'))).toBe(false)
    expect(engine.doesCellHaveFormula(adr('E1'))).toBe(false)
  })
...
describe('#isCellEmpty', () => {
  it('true', () => {
    ...
    expect(engine.isCellEmpty(adr('A1'))).toBe(true)
    expect(engine.isCellEmpty(adr('B1'))).toBe(true)
    expect(engine.isCellEmpty(adr('C1'))).toBe(true)
  })

  it('false', () => {
    const engine = HyperFormula.buildFromArray([['1', '=SUM(1, 2)', '{=TRANSPOSE(A1:A1)}', 'foo']])

    expect(engine.isCellEmpty(adr('A1'))).toBe(false)
    expect(engine.isCellEmpty(adr('B1'))).toBe(false)
    expect(engine.isCellEmpty(adr('C1'))).toBe(false)
    expect(engine.isCellEmpty(adr('D1'))).toBe(false)
  })
...
describe('#isCellPartOfArray', () => {
  it('true', () => {
    ...
    expect(engine.isCellPartOfArray(adr('A1'))).toBe(true)
  })

  it('false', () => {
    const engine = HyperFormula.buildFromArray([['1', '', '=SUM(1, 2)', 'foo']])

    expect(engine.isCellPartOfArray(adr('A1'))).toBe(false)
    expect(engine.isCellPartOfArray(adr('B1'))).toBe(false)
    expect(engine.isCellPartOfArray(adr('C1'))).toBe(false)
    expect(engine.isCellPartOfArray(adr('D1'))).toBe(false)
  })
...
describe('#simpleCellRangeToString', () => {
  ...
  it('should work with second argument `{ includeSheetName: true }`', () => {
    ...
    expect(stringRange).toBe('Sheet0!A1:B2')
  })
})
...
describe('#simpleCellAddressToString', () => {
  ...
  it('should work with second argument `{ includeSheetName: true }`', () => {
    ...
    expect(stringAddress).toBe('Sheet0!A1')
  })
})
...
describe('#simpleCellAddressFromString', () => {
  ...
  it('should return undefined when sheet name contains unicode characters (unquoted sheet name)', () => {
    ...
    expect(engine.simpleCellAddressFromString('あは!C5', 2)).toBeUndefined()
  })
})
...
describe('#simpleCellRangeFromString', () => {
  ...
  it('should return undefined if different sheet names are provided for the start and the end of the range', () => {
    ...
    expect(engine.simpleCellRangeFromString('Sheet0!A1:Sheet1!C1', 42)).toBeUndefined()
  })
})
