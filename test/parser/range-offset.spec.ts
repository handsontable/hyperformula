...
describe('Parser - range offset', () => {
  it('OFFSET - usage with range', () => {
    ...
    const ast = parser.parse('=A1:OFFSET(A1, 1, 1, 1, 1)', absoluteCellAddress(0, 0, 0)).ast as CellRangeAst
    const ast2 = parser.parse('=OFFSET(A1, 1, 1, 1, 1):OFFSET(B2, 1, 1, 1, 1)', absoluteCellAddress(0, 0, 0)).ast as CellRangeAst
    const ast3 = parser.parse('=OFFSET(A1, 1, 1, 1, 1):B3', absoluteCellAddress(0, 0, 0)).ast as CellRangeAst
  ...
  it('OFFSET - range offset not allowed', () => {
    ...
    const ast = parser.parse('=A1:OFFSET(B2, 0, 0, 2, 2)', absoluteCellAddress(0, 0, 0)).ast as ErrorAst
    const ast2 = parser.parse('=OFFSET(A1,0,0,2,2):A2', absoluteCellAddress(0, 0, 0)).ast as ErrorAst
