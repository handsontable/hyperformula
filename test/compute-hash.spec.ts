import { absoluteCellAddress, CellAddress, simpleCellAddress } from '../src/Cell'
...
describe('computeHash', () => {
  ...
  it('cell relative reference', () => {
    ...
    expect(computeFunc(code, absoluteCellAddress(0, 1, 1))).toEqual('=#0#3R-1')
  })

  it('cell absolute reference', () => {
    const code = '=$A$5'

    expect(computeFunc(code, absoluteCellAddress(0, 1, 1))).toEqual('=#0#4A0')
  })

  it('cell absolute col reference', () => {
    const code = '=$A5'

    expect(computeFunc(code, absoluteCellAddress(0, 1, 1))).toEqual('=#0#3AC0')
  })

  it('cell absolute row reference', () => {
    const code = '=A$5'

    expect(computeFunc(code, absoluteCellAddress(0, 1, 1))).toEqual('=#0#4AR-1')
  })

  it('more addresses', () => {
    const code = '=A5+A7'

    expect(computeFunc(code, absoluteCellAddress(0, 1, 1))).toEqual('=#0#3R-1+#0#5R-1')
  })
  ...
  it('cell ref between strings', () => {
    ...
    expect(computeFunc(code, absoluteCellAddress(0, 1, 1))).toEqual('="A5"+#0#2R-1+"A6"')
  })
  ...
  it('cell range', () => {
    ...
    expect(computeFunc(code, absoluteCellAddress(0, 1, 1))).toEqual('=#0#3R-1:#0#14R0')
  })
  ...
  it('support sheets', () => {
    ...
    expect(computeFunc(code, absoluteCellAddress(0, 1, 1))).toEqual('=#1#3R-1')
  })
})
