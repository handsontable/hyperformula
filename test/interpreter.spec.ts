...
describe('Interpreter', () => {
  ...
  it('plus operator', () => {
    const engine = HyperFormula.buildFromArray([['3', '7', '=A1+B1']])

    expect(engine.getCellValue(adr('C1'))).toBe(10)
  })

  it('plus operator', () => {
    const engine = HyperFormula.buildFromArray([['3', '=A1+42']])

    expect(engine.getCellValue(adr('B1'))).toBe(45)
  })

  it('plus operator - int + float', () => {
    const engine = HyperFormula.buildFromArray([['2.0', '=A1 + 3.14']])

    expect(engine.getCellValue(adr('B1'))).toBeCloseTo(5.14)
  })

  it('plus operator - VALUE error on 1st operand', () => {
    const engine = HyperFormula.buildFromArray([['www', '=A1+42']])

    expect(engine.getCellValue(adr('B1'))).toEqual(new CellError(ErrorType.VALUE))
  })

  it('plus operator - VALUE error on 2nd operand', () => {
    const engine = HyperFormula.buildFromArray([['www', '=42+A1']])

    expect(engine.getCellValue(adr('B1'))).toEqual(new CellError(ErrorType.VALUE))
  })

  it('minus operator', () => {
    const engine = HyperFormula.buildFromArray([['3', '=A1-43']])
  ...
  it('times operator', () => {
    const engine = HyperFormula.buildFromArray([['3', '=A1*6']])
  ...
  it('div operator - int result', () => {
    const engine = HyperFormula.buildFromArray([['=10 / 2']])

    expect(engine.getCellValue(adr('A1'))).toEqual(5)
  })

  it('div operator - float result', () => {
    const engine = HyperFormula.buildFromArray([['=5 / 2']])

    expect(engine.getCellValue(adr('A1'))).toEqual(2.5)
  })

  it('div operator - float arg and result', () => {
    const engine = HyperFormula.buildFromArray([['=12 / 2.5']])

    expect(engine.getCellValue(adr('A1'))).toEqual(4.8)
  })

  it('div operator - DIV_ZERO error', () => {
    const engine = HyperFormula.buildFromArray([['=42 / 0']])
  ...
  it('power operator', () => {
    const engine = HyperFormula.buildFromArray([['3', '=2^A1']])

    expect(engine.getCellValue(adr('B1'))).toBe(8)
  })

  it('power operator - VALUE error on 1st operand', () => {
    const engine = HyperFormula.buildFromArray([['www', '=A1^3']])

    expect(engine.getCellValue(adr('B1'))).toEqual(new CellError(ErrorType.VALUE))
  })

  it('power operator - VALUE error on 2nd operand', () => {
    const engine = HyperFormula.buildFromArray([['www', '=2^A1']])
