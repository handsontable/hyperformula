...
describe('Evaluation suspension', () => {
  ...
  it('#1291 - neighboring cell changes of an array formula should be emitted', () => {
    ...
    expect(changes).toContainEqual({address: adr('A1'), newValue: 1})
    expect(changes).toContainEqual({address: adr('A2'), newValue: 2})
    expect(changes).toContainEqual({address: adr('A3'), newValue: 3})
    expect(changes).toContainEqual({address: adr('B1'), newValue: 4})
    expect(changes).toContainEqual({address: adr('C1'), newValue: 5})
  })
})
