...
const engine = await HyperFormula.buildFromArray([
    ...
    await engine.setCellContents(adr('B1'), '35')

    expect(engine.getCellValue(adr('A1'))).not.toEqual(valueBeforeRecomputation)
  })

  it('cell which is dependent on volatile formula is also recomputed', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=RAND()', '42', '=A1'],
    ])
  ...
  it('formula can be recognized as volatile even if entered later', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=A2+A3', '42'],
    ])

    await engine.setCellContents(adr('A1'), '=RAND()')

    const a1 = engine.addressMapping.getCell(adr('A1'))
    expect(engine.dependencyGraph.volatileVertices()).toEqual(new Set([a1]))
  })

  it('volatile vertices should not be recomputed after removing from graph', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=RAND()', '42'],
    ])

    await engine.setCellContents(adr('A1'), '35')

    expect(engine.dependencyGraph.verticesToRecompute()).toEqual(new Set())
  })

  it('volatile formula after moving is still volatile', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=RAND()', '42'],
    ])
