...
describe('Moving rows - checking if its possible', () => {
  it('source top left corner should have valid coordinates', async() => {
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('no if we move to range which overlaps with matrix', async() => {
const engine = await HyperFormula.buildFromArray([
      ['1', '2'],
      ['3', '4'],
      ['=TRANSPOSE(A1:B2)'],
  ...
  it('no if we move beyond sheet size limits ', async() => {
const engine = await HyperFormula.buildFromArray([
      ['1', '2'],
      ['3', '4'],
    ])
...
const engine = await HyperFormula.buildFromSheets({
  ...
  it('should update address in vertex', async() => {
const engine = await HyperFormula.buildFromSheets({
      Sheet1: [
        ['foo'],
        ['=A1'],
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('should throw error trying to move cells beyond sheet limits', async() => {
const engine = await HyperFormula.buildFromArray([
      ['1', '2'],
      ['3', '4'],
    ])
    ...
    await expect((async() => await engine.moveCells(AbsoluteCellRange.spanFrom(adr('A1'), 2, 1), cellInLastColumn))()).rejects.toThrow(new SheetSizeLimitExceededError())
    await expect((async() => await engine.moveCells(AbsoluteCellRange.spanFrom(adr('A1'), 1, 2), cellInLastRow))()).rejects.toThrow(new SheetSizeLimitExceededError())
  })
})
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('overlapped formula with range', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=A2:B2'],
      ['42']
    ])
...
const engine = await HyperFormula.buildFromArray([[1, 2, '=SUM(A1:B1)', '=SUM($A1:B$1)', '=SUM(A$1:$B$1)']])
  ...
  it('move formula and one end of a range', async() => {
const engine = await HyperFormula.buildFromArray([
      [1, 2],
      [null, '=SUM(A1:B1)']
    ])
...
const engine = await HyperFormula.buildFromArray([
  ...
  it('should transform relative column references', async() => {
const engine = await HyperFormula.buildFromArray([
      ['=SUM(C:D)', '', '1', '2']
    ])
