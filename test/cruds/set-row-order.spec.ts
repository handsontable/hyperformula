...
describe('setting row order - checking if it is possible', () => {
  it('should validate numbers for negative rows', () => {
    ...
    expect(engine.isItPossibleToSetRowOrder(0, [[-1, 0]])).toEqual(false)
    expect(() =>
  ...
  it('should validate sources for noninteger values', () => {
    const engine = HyperFormula.buildFromArray([[]])
    expect(engine.isItPossibleToSetRowOrder(0, [[1, 1], [0.5, 0]])).toEqual(false)
    expect(() =>
  ...
  it('should validate sources for values exceeding sheet height', () => {
    ...
    expect(engine.isItPossibleToSetRowOrder(0, [[1, 1], [3,0]])).toEqual(false)
    expect(() =>
  ...
  it('should validate sources to be unique', () => {
    const engine = HyperFormula.buildFromArray([[0], [0], [0]])
    expect(engine.isItPossibleToSetRowOrder(0, [[0, 0],[1,1],[1,2]])).toEqual(false)
    expect(() =>
  ...
  it('should validate sources to be permutation of targets', () => {
    const engine = HyperFormula.buildFromArray([[0], [0], [0]])
    expect(engine.isItPossibleToSetRowOrder(0, [[0, 0],[1,1],[2,1]])).toEqual(false)
    expect(() =>
  ...
  it('should check for matrices', () => {
    ...
    expect(engine.isItPossibleToSetRowOrder(0, [[0, 2],[1,1],[2,0]])).toEqual(false)
    expect(() =>
  ...
  it('should check for matrices only in moved rows', () => {
    ...
    expect(engine.isItPossibleToSetRowOrder(0, [[0, 2],[1,1],[2,0]])).toEqual(true)
    expect(() =>
      engine.setRowOrder(0, [[0, 2], [1, 1], [2, 0]])
    ).not.toThrowError()
  })
...
describe('should correctly work', () => {
  it('should work on static engine', () => {
    ...
    expect(engine.isItPossibleToSetRowOrder(0, [[0, 1],[1,0]])).toEqual(true)
  ...
  it('should work with more complicated permutations', () => {
    ...
    expect(engine.isItPossibleToSetRowOrder(0, [[0, 1],[1,2],[2,0]])).toEqual(true)
  ...
  it('should not move values unnecessarily', () => {
    const engine = HyperFormula.buildFromArray([[1, 2, 3], [4, 5, 6]])
    expect(engine.isItPossibleToSetRowOrder(0, [[0, 0],[1,1]])).toEqual(true)
