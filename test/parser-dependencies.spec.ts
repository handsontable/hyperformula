...
describe('Parsing collecting dependencies', () => {
  it('works for CELL_REFERENCE with relative dependency', () => {
    ...
    const dependencies = parser.parse('=B2', absoluteCellAddress(0, 1, 1)).dependencies
  ...
  it('works with absolute dependencies', () => {
    ...
    const dependencies = parser.parse('=$B$2', absoluteCellAddress(0, 1, 1)).dependencies
  ...
  it('goes inside unary minus', () => {
    ...
    const dependencies = parser.parse('=-B2', absoluteCellAddress(0, 0, 0)).dependencies
  ...
  it('goes inside plus operator', () => {
    ...
    const dependencies = parser.parse('=B2+C3', absoluteCellAddress(0, 0, 0)).dependencies
  ...
  it('goes inside function call arguments', () => {
    ...
    const dependencies = parser.parse('=SUM(B2, C3)', absoluteCellAddress(0, 0, 0)).dependencies
  ...
  it('OFFSET call is correctly found as dependency', () => {
    ...
    const dependencies = parser.parse('=OFFSET(D4, 0, 0)', absoluteCellAddress(0, 1, 1)).dependencies
    expect(dependencies).toEqual([
  ...
  it('COLUMNS arguments are not dependencies', () => {
    ...
    const dependencies = parser.parse('=COLUMNS(A1:B3)', absoluteCellAddress(0, 1, 1)).dependencies
    expect(dependencies).toEqual([])
  })
})
