...
export function benchmark(sheet: string[][], expectedValues: ExpectedValue[], config: Config = defaultConfig) {
  ...
  console.info(`Average: ${averageOverall / 1000}`)
  console.info(`Standard deviation: ${stdDevOverall / 1000}`)
  console.info('---')
  console.info(`Evaluation: ${evaluation.map((v) => (v / 1000))} (in seconds)`)
  console.info(`Median: ${evaluation[Math.trunc(config.numberOfRuns / 2)] / 1000}`)
  console.info('---')
  console.info(`Parsing: ${parsing.map((v) => (v / 1000))}`)
  console.info(`Median: ${parsing[Math.trunc(config.numberOfRuns / 2)] / 1000}`)
  console.info('---')
  console.info(`TopSort: ${topSort.map((v) => (v / 1000))}`)
  console.info(`Median: ${topSort[Math.trunc(config.numberOfRuns / 2)] / 1000}`)
  console.info('---')
  console.info(`Build Graph: ${buildGraph.map((v) => (v / 1000))}`)
  console.info(`Median: ${buildGraph[Math.trunc(config.numberOfRuns / 2)] / 1000}`)
  console.info('\n')
