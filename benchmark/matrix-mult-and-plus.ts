...
export function sheet(): string[][] {
  ...
  const matrix1bottomRightCorner = sheetCellAddressToString({ row: matrixSize - 1, col: matrixSize - 1 })
  const matrix2topLeftCorner = sheetCellAddressToString({ row: 0, col: matrixSize + 1 })
  const matrix2bottomRightCorner = sheetCellAddressToString({ row: matrixSize - 1, col: matrixSize * 2 })

  for (let i = 0; i < matrixSize; i++) {
    const rowToPush: string[] = []
    for (let j = 0; j < matrixSize; j++) {
      rowToPush.push(`{=MMULT(A1:${matrix1bottomRightCorner}, ${matrix2topLeftCorner}:${matrix2bottomRightCorner}) + ${matrix2topLeftCorner}:${matrix2bottomRightCorner}}`)
    }
    sheet.push(rowToPush)
  }
