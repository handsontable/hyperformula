...
export class ParserWithCaching {
   ...
   */
  public parse(text: string, formulaAddress: SimpleCellAddress, sheetContext?: number): ParsingResult {
    ...
    const hash = this.computeHashFromTokens(lexerResult.tokens, formulaAddress, sheetContext)
    ...
    this.addScopeToAbsoluteReferences(astWithNoReversedRanges, sheetContext)
    ...
    switch (ast.type) {
      ...
      case AstNodeType.ROW_RANGE: {
          ...
          const absoluteRefTypes = [
            ...
            CellReferenceType.CELL_REFERENCE_ABSOLUTE_COL,
            CellReferenceType.CELL_REFERENCE_ABSOLUTE_ROW,         
          ]
          if (absoluteRefTypes.includes(ast.start.type) && (ast.start.sheet === undefined)) {
            ast.start = ast.start.withSheet(sheetId)
          }
          if (absoluteRefTypes.includes(ast.end.type) && (ast.end.sheet === undefined)) {
  ...
  public computeHashFromTokens(tokens: IToken[], baseAddress: SimpleCellAddress, sheetContext?: number): string {
    let hash = ''
    let idx = 0
    while (idx < tokens.length) {
      const token = tokens[idx]
      if (tokenMatcher(token, CellReference)) {
        const cellAddress = cellAddressFromString(this.sheetMapping, token.image, baseAddress, sheetContext)
        ...
        const startAddress = columnAddressFromString(this.sheetMapping, start, baseAddress, sheetContext)
        const endAddress = columnAddressFromString(this.sheetMapping, end, baseAddress, sheetContext)
        ...
        const [start, end] = token.image.split(':')
        const startAddress = rowAddressFromString(this.sheetMapping, start, baseAddress, sheetContext)
        const endAddress = rowAddressFromString(this.sheetMapping, end, baseAddress, sheetContext)
        if (startAddress === undefined || endAddress === undefined) {
          hash = hash.concat('!REF')
        } else {
          hash = hash.concat(startAddress.hash(true), ':', endAddress.hash(true))
