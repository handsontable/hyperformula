# `src/DependencyGraph/` — what depends on what

The structure that makes recalculation incremental. Vertices are cells and ranges; edges are dependencies; a topological sort gives the evaluation order.
...
- **Ranges stay collapsed.** Expanding a range into per-cell edges turns a `SUM(A1:A100000)` into 100 000 edges and destroys the performance the engine exists for. If you need per-cell information, ask whether the range vertex can answer instead.
- **Every structural change must keep the mapping consistent.** Adding or removing a row moves addresses; the address mapping, the range mapping, and the array mapping all have to agree afterwards, or a later read resolves to the wrong vertex. Structural changes are paired with `dependencyTransformers/` and `LazilyTransformingAstService.ts`.
...
Structural-change tests are the ones that catch real bugs here: add/remove rows and columns around a formula, move a range across the formula that references it, and assert both the recalculated value **and** the formula text afterwards.
