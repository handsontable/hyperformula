 ...
 */
 ...
 export class Interpreter {
   ...
   public async evaluateAst(ast: Ast, state: InterpreterState): AsyncInterpreterValue {
     let val = await this.evaluateAstWithoutPostprocessing(ast, state)
    ...
    */
   private async evaluateAstWithoutPostprocessing(ast: Ast, state: InterpreterState): AsyncInterpreterValue {
     switch (ast.type) {
       ...
       case AstNodeType.CONCATENATE_OP: {
         const leftResult = await this.evaluateAst(ast.left, state)
         const rightResult = await this.evaluateAst(ast.right, state)
       ...
       case AstNodeType.PLUS_UNARY_OP: {
         const result = await this.evaluateAst(ast.value, state)
       ...
       case AstNodeType.PARENTHESIS: {
         return await this.evaluateAst(ast.expression, state)
       }
       case AstNodeType.ARRAY: {
         ...
         for(const astRow of ast.args) {
           ...
           for(const astIt of astRow) {
             const arr = coerceToRange(await this.evaluateAst(astIt, state))
