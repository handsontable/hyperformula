...
export const DATE_AND_TIME_DOCS: Record<string, FunctionDoc> = {
  ...
  DATEDIF: {
    ...
    parameters: [
      {name: 'Date1', description: 'The earlier (start) date of the period; must not be later than Date2.'},
      {name: 'Date2', description: 'The later (end) date of the period.'},
      {name: 'Unit', description: 'A code selecting the unit of the result: "D", "M", "Y", "MD", "YM", or "YD".'},
    ],
  ...
  DAYS: {
    ...
    parameters: [
      {name: 'Date2', description: 'The end (more recent) date value.'},
      {name: 'Date1', description: 'The start (earlier) date value; the result is Date2 minus Date1, in days.'},
    ],
  ...
  DAYS360: {
    ...
    parameters: [
      {name: 'Date2', description: 'The start date of the 360-day (30-day-month) period.'},
      {name: 'Date1', description: 'The end date of the 360-day period; the result is Date1 minus Date2 measured with 30-day months.'},
      {name: 'Format', description: 'TRUE uses the European 30/360 method; FALSE (default) uses the US (NASD) method.'},
    ],
  ...
  EDATE: {
    ...
    parameters: [
      {name: 'Startdate', description: 'The date value to shift.'},
      {name: 'Months', description: 'The number of months to shift Startdate by; negative values shift backwards.'},
    ],
  ...
  EOMONTH: {
    ...
    parameters: [
      {name: 'Startdate', description: 'The date value to start counting from.'},
      {name: 'Months', description: 'The number of months to add to Startdate before finding the end of that month; negative values go backwards.'},
    ],
  ...
  NETWORKDAYS: {
    ...
    parameters: [
      {name: 'Date1', description: 'The start date of the range.'},
      {name: 'Date2', description: 'The end date of the range.'},
      {name: 'Holidays', description: 'An optional range of dates to exclude from the working-day count, in addition to weekends (Saturday and Sunday).'},
    ],
  ...
  'NETWORKDAYS.INTL': {
    ...
    parameters: [
      {name: 'Date1', description: 'The start date of the range.'},
      {name: 'Date2', description: 'The end date of the range.'},
      {name: 'Mode', description: 'A weekend code (1-7, 11-17; default 1 for Saturday/Sunday) or a 7-character string of 0s and 1s marking weekend days, starting from Monday.'},
      {name: 'Holidays', description: 'An optional range of dates to exclude from the working-day count, in addition to the weekend days.'},
    ],
  ...
  TIMEVALUE: {
    ...
    parameters: [{name: 'Timestring', description: 'A text string representing a time, in one of the formats configured via `timeFormats`.'}],
  ...
  WEEKDAY: {
    ...
    parameters: [
      {name: 'Date', description: 'The date value whose day of the week is returned.'},
      {name: 'Type', description: 'A code selecting the numbering scheme (default 1: Sunday=1; 2: Monday=1; 3: Monday=0; 11-17: week starting on each successive weekday, numbered 1-7).'},
    ],
  ...
  WEEKNUM: {
    ...
    parameters: [
      {name: 'Date', description: 'The date value whose week-of-year number is returned.'},
      {name: 'Type', description: 'A code selecting which weekday starts the week (default 1: Sunday; 2: Monday; 11-17: each successive weekday; 21: ISO week numbering, Monday-based).'},
    ],
  ...
  WORKDAY: {
    ...
    parameters: [
      {name: 'Date', description: 'The start date to count from.'},
      {name: 'Shift', description: 'The number of working days to add (positive) or subtract (negative), skipping weekends (Saturday and Sunday).'},
      {name: 'Holidays', description: 'An optional range of dates to also skip, in addition to weekends.'},
    ],
  ...
  'WORKDAY.INTL': {
    ...
    parameters: [
      {name: 'Date', description: 'The start date to count from.'},
      {name: 'Shift', description: 'The number of working days to add (positive) or subtract (negative).'},
      {name: 'Mode', description: 'A weekend code (1-7, 11-17; default 1 for Saturday/Sunday) or a 7-character string of 0s and 1s marking weekend days, starting from Monday.'},
      {name: 'Holidays', description: 'An optional range of dates to also skip, in addition to the weekend days.'},
    ],
  ...
  YEARFRAC: {
    ...
    parameters: [
      {name: 'Date2', description: 'One of the two boundary dates of the period; HyperFormula automatically reorders Date1/Date2 so the result is never negative.'},
      {name: 'Date1', description: 'The other boundary date of the period.'},
      {name: 'Format', description: 'A basis code selecting the day-count convention: 0 = US 30/360 (default), 1 = actual/actual, 2 = actual/360, 3 = actual/365, or 4 = European 30/360.'},
    ],
