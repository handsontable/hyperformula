  ...
  private commonArgumentsHandling2(ast: ProcedureAst, formulaAddress: SimpleCellAddress, roundingFunction: RoundingFunction): InternalCellValue {
    if (ast.args.length < 1 || ast.args.length > 2) {
      ...
      if (ast.args[1]) {
        ...
        coercedPlaces = coerceScalarToNumber(places, this.interpreter.dateHelper)
      } else {
        coercedPlaces = 0
      }

      const coercedNumberToRound = coerceScalarToNumber(numberToRound, this.interpreter.dateHelper)
