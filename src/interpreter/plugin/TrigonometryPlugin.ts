...
export class TrigonometryPlugin extends FunctionPlugin implements FunctionPluginTypecheck<TrigonometryPlugin>{
   ...
   * @param state
   */
  public acos(ast: ProcedureAst, state: InterpreterState): AsyncInterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('ACOS'), Math.acos)
  }

  public asin(ast: ProcedureAst, state: InterpreterState): AsyncInterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('ASIN'), Math.asin)
  }

  public cos(ast: ProcedureAst, state: InterpreterState): AsyncInterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('COS'), Math.cos)
  }

  public sin(ast: ProcedureAst, state: InterpreterState): AsyncInterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('SIN'), Math.sin)
  }

  public tan(ast: ProcedureAst, state: InterpreterState): AsyncInterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('TAN'), Math.tan)
  }

  public atan(ast: ProcedureAst, state: InterpreterState): AsyncInterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('ATAN'), Math.atan)
  }

  public atan2(ast: ProcedureAst, state: InterpreterState): AsyncInterpreterValue {
  ...
  public sech(ast: ProcedureAst, state: InterpreterState): AsyncInterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('SECH'),
      (arg: number) => 1 / Math.cosh(arg)
    )
  }

  public csch(ast: ProcedureAst, state: InterpreterState): AsyncInterpreterValue {
