...
export class DateTimePlugin extends FunctionPlugin implements FunctionPluginTypecheck<DateTimePlugin>{
  ...
  public date(ast: ProcedureAst, state: InterpreterState): InterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('DATE'), (year, month, day) => {
      ...
      if (y < this.interpreter.dateTimeHelper.getEpochYearZero()) {
        y += this.interpreter.dateTimeHelper.getEpochYearZero()
      }
      ...
      if (this.interpreter.dateTimeHelper.isValidDate(date)) {
        let ret: Maybe<number> = this.interpreter.dateTimeHelper.dateToNumber(date) + (d - 1)
        ret = this.interpreter.dateTimeHelper.getWithinBounds(ret)
        if(ret === undefined) {
  ...
  public eomonth(ast: ProcedureAst, state: InterpreterState): InterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('EOMONTH'), (dateNumber, numberOfMonthsToShift) => {
      const date = this.interpreter.dateTimeHelper.numberToSimpleDate(dateNumber)
      let ret: Maybe<number> = this.interpreter.dateTimeHelper.dateToNumber(this.interpreter.dateTimeHelper.endOfMonth(offsetMonth(date, numberOfMonthsToShift)))
      ret = this.interpreter.dateTimeHelper.getWithinBounds(ret)
      if(ret === undefined) {
        return new CellError(ErrorType.NUM, ErrorMessage.DateBounds)
      }
  ...
  public day(ast: ProcedureAst, state: InterpreterState): InterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('DAY'),
      (dateNumber) => this.interpreter.dateTimeHelper.numberToSimpleDate(dateNumber).day
    )
  }
  ...
  public month(ast: ProcedureAst, state: InterpreterState): InterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('MONTH'),
      (dateNumber) => this.interpreter.dateTimeHelper.numberToSimpleDate(dateNumber).month
    )
  }
  ...
  public year(ast: ProcedureAst, state: InterpreterState): InterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('YEAR'),
      (dateNumber) => this.interpreter.dateTimeHelper.numberToSimpleDate(dateNumber).year
    )
  }
  ...
  public text(ast: ProcedureAst, state: InterpreterState): InterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('TEXT'),
      (numberRepresentation, formatArg) => format(numberRepresentation, formatArg, this.config, this.interpreter.dateTimeHelper)
    )
  }
  ...
  public weekday(ast: ProcedureAst, state: InterpreterState): InterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('WEEKDAY'),
      (day: number, type: number) => {
        const absoluteDay = Math.floor(this.interpreter.dateTimeHelper.relativeNumberToAbsoluteNumber(day))
        if(type===3) {
  ...
  public weeknum(ast: ProcedureAst, state: InterpreterState): InterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('WEEKNUM'),
      (day: number, type: number) => {
        const absoluteDay = Math.floor(this.interpreter.dateTimeHelper.relativeNumberToAbsoluteNumber(day))
        const date = this.interpreter.dateTimeHelper.numberToSimpleDate(day)
        const yearStart = this.interpreter.dateTimeHelper.dateToNumber({year: date.year, month: 1, day: 1})
        const yearStartAbsolute = this.interpreter.dateTimeHelper.relativeNumberToAbsoluteNumber(yearStart)
        if(type === 21) {
  ...
  public datevalue(ast: ProcedureAst, state: InterpreterState): InterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('DATEVALUE'),
      (date: string) => {
        const {dateTime} = this.interpreter.dateTimeHelper.parseDateTimeFromConfigFormats(date)
        ...
        return (instanceOfSimpleTime(dateTime) ? Math.trunc(timeToNumber(dateTime)) : 0) +
          this.interpreter.dateTimeHelper.dateToNumber(dateTime)
      }
    )
  }
  ...
  public timevalue(ast: ProcedureAst, state: InterpreterState): InterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('TIMEVALUE'),
      (date: string) => {
        const dateNumber = this.interpreter.dateTimeHelper.dateStringToDateNumber(date)
    ...
    return this.runFunction(ast.args, state, this.metadata('NOW'),
      () => {
        ...
        return timeToNumber({hours: now.getHours(), minutes: now.getMinutes(), seconds: now.getSeconds()})+
          this.interpreter.dateTimeHelper.dateToNumber({year: now.getFullYear(), month: now.getMonth()+1, day: now.getDate()})
      }
    )
  }
  ...
  public today(ast: ProcedureAst, state: InterpreterState): InterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('TODAY'),
      () => {
        const now = new Date(Date.now())
        return this.interpreter.dateTimeHelper.dateToNumber({year: now.getFullYear(), month: now.getMonth()+1, day: now.getDate()})
      }
    )
  }
  ...
  public edate(ast: ProcedureAst, state: InterpreterState): InterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('EDATE'),
      (dateNumber: number, delta: number) => {
        ...
        let ret: Maybe<number> = this.interpreter.dateTimeHelper.dateToNumber(newDate)
        ret = this.interpreter.dateTimeHelper.getWithinBounds(ret)
        if(ret === undefined) {
          return new CellError(ErrorType.NUM, ErrorMessage.DateBounds)
        }
  ...
  public datedif(ast: ProcedureAst, state: InterpreterState): InterpreterValue {
    return this.runFunction(ast.args, state, this.metadata('DATEDIF'),
      (startDate: number, endDate: number, unit: string) => {
        ...
        const start = this.interpreter.dateTimeHelper.numberToSimpleDate(startDate)
        const end = this.interpreter.dateTimeHelper.numberToSimpleDate(endDate)
        switch(unit) {
          case 'M':
          ...
          case 'MD':
            if(end.day >= start.day) {
              ...
              return this.interpreter.dateTimeHelper.daysInMonth(y, m)+end.day-start.day
            }
          case 'YD':
            if(end.month > start.month || (end.month === start.month && end.day >= start.day)) {
              return Math.floor(endDate) - this.interpreter.dateTimeHelper.dateToNumber({year: end.year, month: start.month, day: start.day})
            } else {
              return Math.floor(endDate)
                - Math.floor(startDate)
                - 365*(end.year-start.year-1)
                - this.interpreter.dateTimeHelper.leapYearsCount(end.year-1)
                + this.interpreter.dateTimeHelper.leapYearsCount(start.year)
            }
          default:
            return new CellError(ErrorType.NUM, ErrorMessage.BadMode)
        }
  ...
  private days360Core = (startDate: number, endDate: number, mode: boolean): number => {
    ...
    if(mode) {
      ...
      [nStart, nEnd] = this.interpreter.dateTimeHelper.toBasisUS(start, end)
    }
    ...
    return this.runFunction(ast.args, state, this.metadata('YEARFRAC'),
      (startDate: number, endDate: number, mode: number) => {
        ...
        switch (mode) {
          ...
          case 1:
            return (endDate-startDate) / this.interpreter.dateTimeHelper.yearLengthForBasis(
              this.interpreter.dateTimeHelper.numberToSimpleDate(startDate),
              this.interpreter.dateTimeHelper.numberToSimpleDate(endDate)
            )
          case 2:
  ...
  private countWorkdays(start: number, end: number, weekendPattern: string, sortedHolidays: number[]): number {
    const absoluteEnd = Math.floor(this.interpreter.dateTimeHelper.relativeNumberToAbsoluteNumber(end))
    const absoluteStart = Math.floor(this.interpreter.dateTimeHelper.relativeNumberToAbsoluteNumber(start))
    let ans = 0
    for(let i=0;i<7;i++) {
  ...
  private simpleRangeToFilteredHolidays(weekendPattern: string, holidays?: SimpleRangeValue): number[] | CellError {
    ...
    return [...new Set(processedHolidays)].sort((a, b) => a-b)
      .filter((arg) => {
      const val = this.interpreter.dateTimeHelper.relativeNumberToAbsoluteNumber(arg)
      const i = (val-1)%7
