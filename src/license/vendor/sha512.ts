...
/**
 * Vendored from `handsontable/license-key`, `src/typed-key/sha512.js`.
...
export function sha512(bytes: number[] | Uint8Array): string {
  ...
  // The message is padded with the 0x80 byte, zeros, and the 128-bit big-endian bit length so
  // the total length is a multiple of 128 bytes.
  ...
  // The supported message sizes fit well within 2^53 bits, so only the two lowest 32-bit words
  // of the 128-bit length field are ever non-zero.
  ...
  const wh = new Array<number>(80)
  const wl = new Array<number>(80)
