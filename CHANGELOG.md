...
- A **breaking change**: the `INDEX` function no longer defaults its third argument to `1`. An omitted third argument now means "every column", so `=INDEX(A1:C3, 2)` returns the whole second row instead of the value in its first column. For a single-row range the only index provided is read as the column number, so `=INDEX(A1:C1, 3)` returns the value of `C1` instead of a `#NUM!` error. Both behaviors match Excel. Where the row number is not a literal and the range has more than one column, as in `=INDEX(A1:C3, B1)`, the whole row cannot be placed in the sheet and the formula returns a `#VALUE!` error; it previously returned the value in the first column. Ranges one column wide, which is where the two-argument form is normally used, are unaffected. [#1754](https://github.com/handsontable/hyperformula/pull/1754)
