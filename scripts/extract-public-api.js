 ...
 * @returns {string|null} File contents, or null when the file is not found.
 */
 ...
 *
 * @param {string} str  Raw parameter-list text.
 * @returns {string[]} Individual parameter strings (whitespace preserved).
 */
 ...
 *
 * @param {string|null|undefined} paramStr  Raw parameter-list string, or null/undefined for empty lists.
 * @returns {{ required: string[], optional: string[] }} Parsed required and optional parameter name lists.
 */
function parseParams(paramStr) {
  ...
  for (const raw of splitParams(paramStr)) {
    ...
    const nameMatch = /^\.{3}?(\w+)/.exec(clean)
 ...
 *
 * @param {string} src  Source text of src/index.ts.
 * @returns {ApiExport[]} All named exports found in the file.
 */
 ...
 *
 * @param {string|null} src  Source text of src/HyperFormula.ts, or null when unavailable.
 * @returns {ApiExport[]} Constructor entry plus all public method entries.
 */
function extractHyperFormulaExports(src) {
  ...
  // Constructor
  const ctorMatch = /\bconstructor\s*\(([^)]*)\)/.exec(src)
  if (ctorMatch) {
 ...
 *
 * @returns {string[]} Top-level directory prefixes included by the TypeScript project.
 */
...
/** @type {string[]} */
const lintScope = extractLintScope()

/** @type {ApiSurface} */
const result = { exports: apiExports, lint_scope: lintScope }
